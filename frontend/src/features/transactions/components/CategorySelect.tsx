    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <ChevronsUpDown size={14} style={{ color: 'var(--ff-text-muted)', flexShrink: 0 }} />
        </button>
      </PopoverTrigger>
      ...
      >
        ...
        >
          <Search size={13} style={{ color: 'var(--ff-text-muted)', flexShrink: 0 }} />
          <input
            type="text"
            value={search}
        ...
        <div className="max-h-56 overflow-y-auto py-1">
          {nullable && (
            <button
              ...
              onMouseEnter={e => { if (value) e.currentTarget.style.background = 'var(--ff-bg-elevated)' }}
              onMouseLeave={e => { if (value) e.currentTarget.style.background = 'transparent' }}
            >
              <Check size={13} style={{ flexShrink: 0, opacity: !value ? 1 : 0 }} />
              {nullLabel}
            </button>
          )}

          {filtered.length === 0 ? (
            <p className="text-sm text-center py-4" style={{ color: 'var(--ff-text-muted)' }}>
            ...
            filtered.map(c => (
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors"
                style={{
                  color:      value === c.id ? 'var(--ff-emerald)' : 'var(--ff-text-secondary)',
                  background: value === c.id ? 'var(--ff-emerald-subtle)' : 'transparent',
                }}
                onMouseEnter={e => { if (value !== c.id) e.currentTarget.style.background = 'var(--ff-bg-elevated)' }}
                onMouseLeave={e => { if (value !== c.id) e.currentTarget.style.background = 'transparent' }}
              >
                <Check size={13} style={{ flexShrink: 0, opacity: value === c.id ? 1 : 0 }} />
