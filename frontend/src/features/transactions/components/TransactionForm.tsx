...
export default function TransactionForm({ transaction, onClose }: TransactionFormProps) {
  ...
  function handleSubmit() {
    ...
    if (isEditing && devePerguntar) {
      // Exibe modal de propagação antes de salvar
  ...
  // Executa o update com ou sem propagação
  ...
  return (
    <>
      ...
      {/* Modal de propagação para recorrentes */}
      {showPropagateModal && (
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                ...
                disabled={isPending}
                className="w-full px-4 py-3 rounded-xl border border-indigo-500/30 bg-indigo-500/5 text-sm font-medium hover:bg-indigo-500/10 transition-colors text-left disabled:opacity-50"
              >
                <p className="font-semibold text-indigo-400">Esta e todas as futuras</p>
                <p className="text-xs text-slate-400 mt-0.5">
                  Aplica as alterações de valor e descrição a todas as ocorrências futuras do grupo.
                </p>
              </button>
              <button
