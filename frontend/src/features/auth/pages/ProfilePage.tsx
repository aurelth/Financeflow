...
import { z } from "zod";
import { Eye, EyeOff, Loader2, Lock, Settings, User, Camera, Trash2, X } from 'lucide-react'
import { useState, useEffect, useRef } from "react";
import { useTranslation } from "react-i18next";
import { useUserProfile, useUpdateProfile, useChangePassword, useUploadAvatar, useDeleteAvatar } from '../api/useAuth'
...
export default function ProfilePage() {
  ...
  const avatarBaseUrl = import.meta.env.VITE_API_URL ?? "";
  const currentAvatar =
    avatarPreview ??
    (profile?.avatarUrl
      ? `${avatarBaseUrl}/uploads/avatars/${profile.avatarUrl}`
      : null);
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      >
        <div className="flex items-center gap-2 mb-5">
          <Camera size={16} style={{ color: "var(--ff-emerald)" }} />
          <h2
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="relative flex-shrink-0">
            <div
              ...
              style={{
                background: currentAvatar ? "transparent" : "var(--ff-emerald)",
                color: "var(--ff-emerald-subtle)",
                border: "2px solid var(--ff-border)",
                cursor: currentAvatar ? "pointer" : "default",
              }}
              onClick={() => currentAvatar && setShowZoom(true)}
              title={currentAvatar ? "Clique para ampliar" : undefined}
            >
              {currentAvatar ? (
                <img
                  src={currentAvatar}
          ...
          <div className="flex flex-col gap-2">
            <button
              ...
              className="flex items-center gap-2 px-4 h-9 rounded-xl text-sm font-medium transition-colors disabled:opacity-50"
              style={{
                background: "var(--ff-emerald)",
                color: "var(--ff-emerald-subtle)",
              }}
              onMouseEnter={onHoverEnterBtnEmerald}
              onMouseLeave={onHoverLeaveBtnEmerald}
            >
              <Camera size={14} />
            ...
            <p className="text-xs" style={{ color: "var(--ff-text-muted)" }}>
              JPEG, PNG ou WebP. Máximo 5MB. Redimensionado para 256×256px.
            </p>
          </div>
        </div>
      ...
      >
        ...
        <p className="text-xs mt-4" style={{ color: "var(--ff-text-muted)" }}>
          ...
          <a
            href="mailto:contato@financeflowapp.io"
      ...
      >
        ...
        <form
          onSubmit={prefForm.handleSubmit((d) => updateProfile(d))}
          className="space-y-4"
        >
