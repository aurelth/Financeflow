...
describe('AssistantPage', () => {
  ...
  it('deve renderizar o título da página', () => {
    renderPage()
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Assistente IA')
  })

  it('deve exibir mensagem de boas-vindas quando sem mensagens', () => {
    renderPage()
    expect(screen.getByText(/Olá! Sou o seu assistente financeiro/i)).toBeInTheDocument()
  })

  it('deve exibir sugestões de perguntas no estado inicial', () => {
    renderPage()
    expect(screen.getByText('Quanto gastei este mês?')).toBeInTheDocument()
    expect(screen.getByText('Estou dentro do meu orçamento?')).toBeInTheDocument()
    expect(screen.getByText('Quais são minhas maiores despesas?')).toBeInTheDocument()
  })
  ...
  it('deve exibir mensagem de erro quando error não é null', () => {
    mockUseAssistant.error = 'Não foi possível obter uma resposta. Tente novamente.'

    renderPage()

    expect(screen.getByText('Não foi possível obter uma resposta. Tente novamente.')).toBeInTheDocument()
  })
